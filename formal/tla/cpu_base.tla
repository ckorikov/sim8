...
\* Vector Unit opcodes (v3)
\* Synchronous
OP_VSET_IMM16 == 163   OP_VSET_GPR == 164   OP_VSET_MEM == 165
OP_VSET_MEMI == 166
OP_VFSTAT == 167       OP_VFCLR == 168        OP_VWAIT == 169
\* Asynchronous
OP_VADD == 170   OP_VSUB == 171   OP_VMUL == 172   OP_VDIV == 173
...
\* --- Vector Unit Helpers (v3) ---

\* VFM byte decode
VFM_fmt(vfm)  == vfm % 8
VFM_mode(vfm) == (vfm \div 8) % 4
VFM_cond(vfm) == vfm \div 32

\* Vector format element size in bytes
VFmtBytes(fmt) ==
    IF fmt = 0 THEN 4          \* F32
    ELSE IF fmt \in {1, 2} THEN 2  \* F16, BF16
    ELSE IF fmt \in {3, 4, 5, 6} THEN 1  \* OFP8, UINT8, INT8
    ELSE 0

\* Register operand byte decode
VRegs_dst(regs)  == (regs \div 64) % 4
VRegs_src1(regs) == (regs \div 16) % 4
VRegs_src2(regs) == (regs \div 4) % 4

\* Read vector pointer register by code (0=VA, 1=VB, 2=VC, 3=VM)
VPtrValue(code) ==
    CASE code = 0 -> VA_reg [] code = 1 -> VB_reg
      [] code = 2 -> VC_reg [] code = 3 -> VM_reg [] OTHER -> 0

\* VFM validation: returns 0 if valid, ERR_VU_FORMAT if invalid
ValidateVFM(vfm, op) ==
    LET fmt == VFM_fmt(vfm)
        mode == VFM_mode(vfm)
        cond == VFM_cond(vfm)
...
\* Vector stride: VL * element_size
VStride(vl, fmt) == vl * VFmtBytes(fmt)
...
VuCommand(op, vfm, regs, vl, imm) ==
    <<op, VFM_fmt(vfm), VFM_mode(vfm), VFM_cond(vfm),
      VPtrValue(VRegs_dst(regs)), VPtrValue(VRegs_src1(regs)),
      VPtrValue(VRegs_src2(regs)), VM_reg, vl, imm>>

\* VU command fields (index into command tuple)
VU_OP == 1   VU_FMT == 2   VU_MODE == 3   VU_COND == 4
VU_DST == 5  VU_S1 == 6    VU_S2 == 7     VU_MASK == 8
VU_VL == 9   VU_IMM == 10

\* VU Fault helper (for async faults — stores code, flushes queue)
VuFault(err) ==
    /\ vu_fault' = err
    /\ vu_queue' = <<>>
