...
describe("DB directive", () => {
    ...
    it("DB OFP8 E4M3 overflow (>448) → assembler error", () => {
        const err = asmError("DB 500.0_o3");
        expect(err.message).toMatch(/Float value out of range/);
        expect(err.message).toMatch(/E4M3/);
    });

    it("FMOV.O3 with overflow immediate → assembler error", () => {
        const err = asmError("FMOV.O3 FQA, 500.0");
        expect(err.message).toMatch(/Float value out of range/);
        expect(err.message).toMatch(/E4M3/);
    });
...
describe("complex programs", () => {
    ...
    it("program with labels and conditional jump", () => {
        const src = ["MOV A, 5", "loop: DEC A", "JNZ loop", "HLT"].join("\n");
        ...
        // HLT      = [0]         (1 byte,  pos 7)
        expect(r.code).toEqual([Op.MOV_REG_CONST, Reg.A, 5, Op.DEC_REG, Reg.A, Op.JNZ_ADDR, 3, Op.HLT]);
    });

    it("FP program with label reference", () => {
        const src = ["JMP start", "data: DB 0, 0, 0, 0", "start: FMOV.F FA, [data]", "HLT"].join("\n");
        const r = assemble(src);
        ...
        // HLT = [0]
        expect(r.code).toEqual([Op.JMP_ADDR, 6, 0, 0, 0, 0, Op.FMOV_FP_ADDR, encodeFpm(0, 0, FP_FMT_F), 2, Op.HLT]);
    });
});
