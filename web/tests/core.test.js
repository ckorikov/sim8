...
describe("MOV instructions", () => {
    ...
    it("MOV [B+0], 42 (regaddr-const): stores immediate to indirect", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.B, 0x50, Op.MOV_REGADDR_CONST, RA_B_0, 42, Op.HLT]);
        cpu.run();
        expect(cpu.mem.get(0x50)).toBe(42);
    });
...
describe("ADD/SUB/CMP", () => {
    ...
    it("ADD A, B: 100+50=150, Z=false, C=false", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 100, Op.MOV_REG_CONST, Reg.B, 50, Op.ADD_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
    ...
    it("ADD with carry: 200+100=44 (mod 256), C=true", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 200, Op.MOV_REG_CONST, Reg.B, 100, Op.ADD_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
    ...
    it("ADD A, [addr]: addr variant", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 10, Op.ADD_REG_ADDR, Reg.A, 0x50, Op.HLT]);
        cpu.mem.set(0x50, 15);
        cpu.run();
    ...
    it("ADD A, 10: const variant", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 5, Op.ADD_REG_CONST, Reg.A, 10, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(15);
    });

    it("SUB A, B: 100-50=50", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 100, Op.MOV_REG_CONST, Reg.B, 50, Op.SUB_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
    ...
    it("SUB underflow: 10-20=246 (mod 256), C=true", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 10, Op.MOV_REG_CONST, Reg.B, 20, Op.SUB_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
    ...
    it("CMP equal: Z=true, C=false, A unchanged", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 50, Op.MOV_REG_CONST, Reg.B, 50, Op.CMP_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
    ...
    it("CMP less: Z=false, C=true", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 10, Op.MOV_REG_CONST, Reg.B, 50, Op.CMP_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
    ...
    it("CMP greater: Z=false, C=false", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 50, Op.MOV_REG_CONST, Reg.B, 10, Op.CMP_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(50);
        expect(cpu.zero).toBe(false);
        expect(cpu.carry).toBe(false);
    ...
    it("CMP with const: CMP A, 42", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 42, Op.CMP_REG_CONST, Reg.A, 42, Op.HLT]);
        cpu.run();
        expect(cpu.zero).toBe(true);
        expect(cpu.carry).toBe(false);
    });
...
describe("INC/DEC", () => {
    ...
    it("INC A: 41 -> 42, Z=false, C=false", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 41, Op.INC_REG, Reg.A, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(42);
        expect(cpu.zero).toBe(false);
        expect(cpu.carry).toBe(false);
    ...
    it("INC 255 -> 0: C=true, Z=true", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 255, Op.INC_REG, Reg.A, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(0);
        expect(cpu.carry).toBe(true);
        expect(cpu.zero).toBe(true);
    ...
    it("DEC A: 1 -> 0, Z=true, C=false", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 1, Op.DEC_REG, Reg.A, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(0);
        expect(cpu.zero).toBe(true);
        expect(cpu.carry).toBe(false);
...
describe("MUL/DIV", () => {
    ...
    it("MUL reg: A=10 * B=5 -> A=50, C=false", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 10, Op.MOV_REG_CONST, Reg.B, 5, Op.MUL_REG, Reg.B, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(50);
        expect(cpu.carry).toBe(false);
    });
    ...
    it("MUL overflow: A=100 * 10 -> A=232, C=true", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 100, Op.MUL_CONST, 10, Op.HLT]);
        cpu.run();
    ...
    it("DIV reg: A=50 / B=5 -> A=10, C=false", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 50, Op.MOV_REG_CONST, Reg.B, 5, Op.DIV_REG, Reg.B, Op.HLT]);
        cpu.run();
    ...
    it("DIV by zero: FAULT(DIV_ZERO)", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 50, Op.DIV_CONST, 0, Op.HLT]);
        cpu.step(); // MOV
    ...
    it("MUL const variant", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 3, Op.MUL_CONST, 7, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(21);
    });

    it("DIV const variant", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 100, Op.DIV_CONST, 10, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(10);
    });
});
...
describe("Jump instructions", () => {
    ...
    it("JMP addr: IP = target", () => {
        ...
        cpu.load([
            Op.JMP_ADDR,
            6, // bytes 0-1: jump to addr 6
            Op.MOV_REG_CONST,
            Reg.A,
            1, // bytes 2-4: should be skipped
            Op.HLT, // byte 5
            Op.MOV_REG_CONST,
            Reg.A,
            99, // bytes 6-8: target
    ...
    it("JA taken: C=false AND Z=false -> jump (above)", () => {
        cpu.load([
            ...
            Op.CMP_REG_REG,
            Reg.A,
            Reg.B, // 50 > 10: Z=false, C=false
            Op.JA_ADDR,
            15, // taken → addr 15
            Op.MOV_REG_CONST,
            Reg.A,
            1, // skipped (addr 11-13)
            Op.HLT, // addr 14
            Op.MOV_REG_CONST,
            Reg.A,
            99, // addr 15
...
describe("Stack operations", () => {
    ...
    it("PUSH reg: SP decrements, value stored at old SP", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 42, Op.PUSH_REG, Reg.A, Op.HLT]);
        cpu.run();
    ...
    it("PUSH const: pushes immediate value", () => {
        cpu.load([Op.PUSH_CONST, 42, Op.HLT]);
        cpu.run();
        expect(cpu.sp).toBe(SP_INIT - 1);
        expect(cpu.mem.get(SP_INIT)).toBe(42);
    });
    ...
    it("POP reg: SP increments, register gets value", () => {
        cpu.load([Op.PUSH_CONST, 42, Op.POP_REG, Reg.A, Op.HLT]);
        cpu.run();
    ...
    it("Stack overflow: SP=0, PUSH -> FAULT(STACK_OVERFLOW)", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.SP, 0, Op.PUSH_CONST, 42, Op.HLT]);
        cpu.step(); // MOV SP, 0
        cpu.step(); // PUSH -> FAULT
        expect(cpu.state).toBe(CpuState.FAULT);
    ...
    it("Stack underflow: SP=231, POP -> FAULT(STACK_UNDERFLOW)", () => {
        cpu.load([Op.POP_REG, Reg.A, Op.HLT]);
        // SP starts at 231
        cpu.step();
        expect(cpu.state).toBe(CpuState.FAULT);
    ...
    it("CALL addr: pushes return address, jumps to target", () => {
        ...
        cpu.load([
            Op.CALL_ADDR,
            8, // bytes 0-1; return addr = 2
            Op.MOV_REG_CONST,
            Reg.A,
            1, // bytes 2-4: should be reached on RET
            Op.HLT, // byte 5
            0,
            0, // padding to byte 7
            Op.MOV_REG_CONST,
...
describe("Bitwise operations", () => {
    ...
    it("AND A, B: 0xFF & 0x0F = 0x0F, C=false", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 0xff, Op.MOV_REG_CONST, Reg.B, 0x0f, Op.AND_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
    ...
    it("OR A, B: 0xF0 | 0x0F = 0xFF", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 0xf0, Op.MOV_REG_CONST, Reg.B, 0x0f, Op.OR_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
    ...
    it("XOR A, B: 0xFF ^ 0xFF = 0, Z=true", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 0xff, Op.MOV_REG_CONST, Reg.B, 0xff, Op.XOR_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(0);
        expect(cpu.zero).toBe(true);
        expect(cpu.carry).toBe(false);
    ...
    it("NOT A: 0x55 -> 0xAA, C=false", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 0x55, Op.NOT_REG, Reg.A, Op.HLT]);
        cpu.run();
    ...
    it("AND const variant", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 0xff, Op.AND_REG_CONST, Reg.A, 0x0f, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(0x0f);
    });

    it("OR addr variant", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 0xf0, Op.OR_REG_ADDR, Reg.A, 0x50, Op.HLT]);
    ...
    it("XOR const variant", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 0xff, Op.XOR_REG_CONST, Reg.A, 0xaa, Op.HLT]);
        cpu.run();
...
describe("Shift operations", () => {
    ...
    it("SHL A, B: value=1, count=3 -> 8", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 1, Op.MOV_REG_CONST, Reg.B, 3, Op.SHL_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(8);
        expect(cpu.carry).toBe(false);
    });
    ...
    it("SHL carry: 0x80 << 1 = 0, C=true, Z=true", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 0x80, Op.SHL_REG_CONST, Reg.A, 1, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(0);
        expect(cpu.carry).toBe(true);
        expect(cpu.zero).toBe(true);
    ...
    it("SHR A, B: value=8, count=3 -> 1", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 8, Op.MOV_REG_CONST, Reg.B, 3, Op.SHR_REG_REG, Reg.A, Reg.B, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(1);
        expect(cpu.carry).toBe(false);
    });
    ...
    it("SHR carry: 1 >> 1 = 0, C=true, Z=true", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 1, Op.SHR_REG_CONST, Reg.A, 1, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(0);
        expect(cpu.carry).toBe(true);
        expect(cpu.zero).toBe(true);
    ...
    it("SHL by 0: value unchanged", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 42, Op.SHL_REG_CONST, Reg.A, 0, Op.HLT]);
        cpu.run();
        expect(cpu.a).toBe(42);
        expect(cpu.zero).toBe(false);
    });
    ...
    it("SHR addr variant", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 16, Op.SHR_REG_ADDR, Reg.A, 0x50, Op.HLT]);
        cpu.mem.set(0x50, 2);
        cpu.run();
        expect(cpu.a).toBe(4);
    });
...
describe("Fault conditions", () => {
    ...
    it("FAULT from DIV 0 at known IP", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 10, Op.DIV_CONST, 0, Op.HLT]);
        cpu.step(); // MOV
        cpu.step(); // DIV 0
        expect(cpu.state).toBe(CpuState.FAULT);
        expect(cpu.a).toBe(ErrorCode.DIV_ZERO);
...
describe("FPU basics", () => {
    ...
    it("FMOV store to memory: mem[addr] = FA.f32", () => {
        ...
        cpu.load([Op.FMOV_FP_ADDR, FPM_FA_F32, 0x50, Op.FMOV_ADDR_FP, FPM_FA_F32, 0x60, Op.HLT]);
    ...
    it("FMOV reg-reg: copy FA to FB", () => {
        cpu.load([Op.FMOV_FP_ADDR, FPM_FA_F32, 0x50, Op.FMOV_RR, FPM_FB_F32, FPM_FA_F32, Op.HLT]);
    ...
    it("FSUB: 10.0 - 3.0 = 7.0", () => {
        cpu.load([Op.FMOV_FP_ADDR, FPM_FA_F32, 0x50, Op.FSUB_FP_ADDR, FPM_FA_F32, 0x60, Op.HLT]);
    ...
    it("FMUL: 4.0 * 3.0 = 12.0", () => {
        cpu.load([Op.FMOV_FP_ADDR, FPM_FA_F32, 0x50, Op.FMUL_FP_ADDR, FPM_FA_F32, 0x60, Op.HLT]);
    ...
    it("FDIV: 10.0 / 2.0 = 5.0", () => {
        cpu.load([Op.FMOV_FP_ADDR, FPM_FA_F32, 0x50, Op.FDIV_FP_ADDR, FPM_FA_F32, 0x60, Op.HLT]);
        storeFloat32(cpu.mem, 0x50, 10.0);
        storeFloat32(cpu.mem, 0x60, 2.0);
        cpu.run();
        expect(cpu.fpu.fa).toBeCloseTo(5.0, 5);
    ...
    it("FCMP equal: Z=true", () => {
        cpu.load([Op.FMOV_FP_ADDR, FPM_FA_F32, 0x50, Op.FCMP_FP_ADDR, FPM_FA_F32, 0x60, Op.HLT]);
        storeFloat32(cpu.mem, 0x50, 3.0);
        storeFloat32(cpu.mem, 0x60, 3.0);
        cpu.run();
        expect(cpu.zero).toBe(true);
    ...
    it("FABS: abs(-5.0) = 5.0", () => {
        cpu.load([Op.FMOV_FP_ADDR, FPM_FA_F32, 0x50, Op.FABS_FP, FPM_FA_F32, Op.HLT]);
        storeFloat32(cpu.mem, 0x50, -5.0);
        cpu.run();
        expect(cpu.fpu.fa).toBeCloseTo(5.0, 5);
    });
    ...
    it("FNEG: neg(3.0) = -3.0", () => {
        cpu.load([Op.FMOV_FP_ADDR, FPM_FA_F32, 0x50, Op.FNEG_FP, FPM_FA_F32, Op.HLT]);
        storeFloat32(cpu.mem, 0x50, 3.0);
        cpu.run();
    ...
    it("FSQRT: sqrt(4.0) = 2.0", () => {
        cpu.load([Op.FMOV_FP_ADDR, FPM_FA_F32, 0x50, Op.FSQRT_FP, FPM_FA_F32, Op.HLT]);
        storeFloat32(cpu.mem, 0x50, 4.0);
        cpu.run();
    ...
    it("FITOF: integer 42 -> float 42.0", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 42, Op.FITOF_FP_GPR, FPM_FA_F32, Reg.A, Op.HLT]);
        cpu.run();
        expect(cpu.fpu.fa).toBeCloseTo(42.0, 5);
    });

    it("FFTOI: float 42.0 -> integer 42", () => {
        cpu.load([Op.FMOV_FP_ADDR, FPM_FA_F32, 0x50, Op.FFTOI_GPR_FP, FPM_FA_F32, Reg.A, Op.HLT]);
    ...
    it("FSTAT: reads FPSR into GPR", () => {
        cpu.load([Op.FSTAT_GPR, Reg.A, Op.HLT]);
        cpu.run();
        // FPSR starts at 0
        expect(cpu.a).toBe(0);
    });
    ...
    it("FCFG: reads FPCR into GPR", () => {
        cpu.load([Op.FCFG_GPR, Reg.A, Op.HLT]);
        cpu.run();
        // FPCR starts at 0
        expect(cpu.a).toBe(0);
    });
    ...
    it("FCVT: convert between formats (FA.f32 -> FHA.f16)", () => {
        cpu.load([Op.FMOV_FP_ADDR, FPM_FA_F32, 0x50, Op.FCVT_FP_FP, FPM_FHA_F16, FPM_FA_F32, Op.HLT]);
        storeFloat32(cpu.mem, 0x50, 1.0);
        cpu.run();
...
describe("display()", () => {
    ...
    it('write chars via MOV: "Hi"', () => {
        ...
        cpu.load([Op.MOV_ADDR_CONST, IO_START, H, Op.MOV_ADDR_CONST, IO_START + 1, i, Op.HLT]);
        cpu.run();
        const disp = cpu.display();
...
describe("CPU step counting", () => {
    ...
    it("steps and cycles increment after each instruction", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 1, Op.MOV_REG_CONST, Reg.B, 2, Op.HLT]);
        cpu.step(); // MOV A, 1
    ...
    it("HLT does not increment step count", () => {
        cpu.load([Op.MOV_REG_CONST, Reg.A, 1, Op.HLT]);
        cpu.run();
