/**
 * IEEE 754 floating-point format encode/decode (format-specific functions).
 *
...
export const NO_EXC = Object.freeze({
    invalid: false,
    divZero: false,
    overflow: false,
    underflow: false,
...
export function exc({ invalid = false, divZero = false, overflow = false, underflow = false, inexact = false } = {}) {
...
export function nextafter(from, to) {
...
/** Pack a 16-bit integer into a 2-byte little-endian Uint8Array. */
function _u16ToLE(bits) {
    return new Uint8Array([bits & 0xff, (bits >> 8) & 0xff]);
}

/** Pack a 32-bit integer into a 4-byte little-endian Uint8Array. */
function _u32ToLE(bits) {
    return new Uint8Array([bits & 0xff, (bits >> 8) & 0xff, (bits >> 16) & 0xff, (bits >> 24) & 0xff]);
}

/** Extract sign and absolute value, treating -0 as negative. Returns [sign, absVal]. */
function _extractSign(value) {
    if (value < 0 || (value === 0 && Object.is(value, -0))) return [1, -value];
    return [0, value];
}
...
const _MIN_NORMAL_F16 = 6.103515625e-5; // 2^-14
const _OVERFLOW_THRESH_F32 = 3.4028235677973366e38;
...
export function encodeFloat32(value, rm = 0) {
    if (Number.isNaN(value) || !Number.isFinite(value) || value === 0) {
        ...
        _f32arr[0] = value;
        return { data: _u32ToLE(_u32arr[0]), exc: NO_EXC };
    }
    ...
    if (Math.abs(value) >= _OVERFLOW_THRESH_F32) {
        _f32arr[0] = value < 0 ? -Infinity : Infinity;
        return { data: _u32ToLE(_u32arr[0]), exc: exc({ overflow: true, inexact: true }) };
    }

    _f32arr[0] = value;
    const f32Bits = _u32arr[0];
    const rt = _f32arr[0];
    const data = _u32ToLE(f32Bits);
...
export function encodeFloat16(value, rm = 0) {
    if (Number.isNaN(value)) {
        return { data: _u16ToLE(0x7e00), exc: NO_EXC };
    }
    if (!Number.isFinite(value)) {
        return { data: _u16ToLE(value > 0 ? 0x7c00 : 0xfc00), exc: NO_EXC };
    }
    if (value === 0) {
        return { data: _u16ToLE(Object.is(value, -0) ? 0x8000 : 0x0000), exc: NO_EXC };
    }
    ...
    if (Math.abs(value) >= _OVERFLOW_THRESH_F16) {
        return { data: _u16ToLE(value < 0 ? 0xfc00 : 0x7c00), exc: exc({ overflow: true, inexact: true }) };
    }

    const f16Bits = _encodeFloat16Rne(value);
    const data = _u16ToLE(f16Bits);
...
export function encodeBfloat16(value, rm = 0) {
    if (Number.isNaN(value)) {
        return { data: _u16ToLE(0x7fc0), exc: NO_EXC };
    }
    if (!Number.isFinite(value)) {
        return { data: _u16ToLE(value > 0 ? 0x7f80 : 0xff80), exc: NO_EXC };
    }
    if (value === 0) {
        // Preserve sign of zero: get from float32
        _f32arr[0] = value;
        return { data: _u16ToLE((_u32arr[0] >>> 16) & 0xffff), exc: NO_EXC };
    }
    ...
    if (Math.abs(value) >= _OVERFLOW_THRESH_F32) {
        return { data: _u16ToLE(value < 0 ? 0xff80 : 0x7f80), exc: exc({ overflow: true, inexact: true }) };
    }
...
export function encodeOfp8E4M3(value, rm = 0) {
    ...
    const [sign, absVal] = _extractSign(value);

    if (!Number.isFinite(absVal)) {
        // No Inf in E4M3 -- saturate to max finite
        return { data: new Uint8Array([(sign << 7) | 0x7e]), exc: exc({ overflow: true, inexact: true }) };
    }
    if (absVal === 0) {
        return { data: new Uint8Array([sign << 7]), exc: NO_EXC };
    }
    if (absVal > _E4M3_MAX_FINITE) {
        return { data: new Uint8Array([(sign << 7) | 0x7e]), exc: exc({ overflow: true, inexact: true }) };
    }
...
export function encodeOfp8E5M2(value, rm = 0) {
    ...
    if (!Number.isFinite(absVal)) {
        return { data: new Uint8Array([(sign << 7) | (_E5M2_MAX_EXP << _E5M2_MANT_BITS)]), exc: NO_EXC };
    }
    if (absVal === 0) {
        return { data: new Uint8Array([sign << 7]), exc: NO_EXC };
    }
...
function _encodeMiniFloat(absVal, sign, expBits, mantBits, bias, hasInf, nanPattern, rm) {
    ...
    if (biasedExp <= 0) {
        ...
        return _overflowResult(sign, expBits, mantBits, maxExp, maxNormalBiased, hasInf, nanPattern, rm);
    } else {
        // Normal number
