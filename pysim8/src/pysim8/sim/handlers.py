...
from pysim8.isa import BY_CODE_FP, BY_CODE_MU, BY_CODE_VU, Op, Reg, decode_regaddr
...
class HandlersMixin:
    ...
    def _validate_dispatch_complete(d: dict[Op, Handler]) -> None:
        ...
        _mu_ops: set[Op] = {Op(c) for c in BY_CODE_MU}
        all_ops: set[Op] = {op for op in Op if op != Op.HLT}
        missing = all_ops - _fp_ops - _vu_ops - _mu_ops - set(d)
        if missing:
